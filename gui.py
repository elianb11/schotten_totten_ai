...
def displayMainMenu():
    ...
    window = Tk()
    window.title('Schotten Totten')

    main_frame = Frame(window, height=MAIN_WIN_H,
                       width=MAIN_WIN_W, borderwidth=3, relief=GROOVE,)
    main_frame.pack(side=TOP)

    C = Canvas(window, height=MAIN_WIN_H, width=MAIN_WIN_W)

    main_image = PhotoImage(
        file='resources/schotten_totten_main_menu.png')
    background_label = Label(main_frame, image=main_image)
    background_label.place(x=0, y=0, relwidth=1, relheight=1)

    checkbutton = Checkbutton(
        window, text="Expert Mode", height=3, width=20, onvalue=True, offvalue=False)
    ...
    bouton2 = Button(main_frame, text="Player 1 VS A.I.",
                     height=3, width=20, command=window.quit)
